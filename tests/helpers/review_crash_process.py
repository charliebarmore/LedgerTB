...
def run_worker(fixture, database_path):
    ready = fixture.with_suffix('.ready')
    ready.unlink(missing_ok=True)
    command = [sys.executable, str(Path(__file__).with_name('review_crash_worker.py')), str(fixture)]
    ...
    try:
        while not ready.exists() and process.poll() is None:
            if time.monotonic() >= deadline:
                raise subprocess.TimeoutExpired('review crash worker startup', allowance)
            time.sleep(.05)
