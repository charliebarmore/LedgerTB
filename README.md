...
- **Linux**: no packaged build yet. From source, the app also runs in its own window instead of a browser tab: inside the virtual environment from the quickstart, run `pip install "pywebview>=5.0,<7.0" PyGObject`, then `python desktop.py`. The window uses the system's WebKitGTK 4.1 and GTK 3, and PyGObject compiles during install, so it also needs a C compiler and the GLib and cairo headers. Verified on Arch-based Omarchy (2026-09-22) with `webkit2gtk-4.1`, `gtk3` and `base-devel` installed. On Debian/Ubuntu the matching packages should be `libwebkit2gtk-4.1-0`, `gir1.2-webkit2-4.1`, `libgirepository-2.0-dev`, `libcairo2-dev`, `pkg-config` and `build-essential`, but that has not been tested.
