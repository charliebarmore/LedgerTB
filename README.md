...
- **Assistant access (MCP)**: opt-in, **read-only** MCP server so Claude Desktop or Claude Code can query your books — trial balance, ledgers, entry search, the integrity sweep. Read-only is enforced by the database, not by promises. See `docs/MCP.md`.
