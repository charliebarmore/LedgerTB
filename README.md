...
- **Assistant access (MCP)**: opt-in MCP server so Claude Desktop or Claude Code can query your books — trial balance, ledgers, entry search, the integrity sweep — file **draft entries** that only a human can approve, and **stage imports from any statement format** (the assistant normalizes; you review and post with full duplicate protection). The ledger itself is unreachable to the assistant, enforced by the database engine, not by promises. See `docs/MCP.md`.
